"""
    Script créé pour consolider les fonctions utiles utilisées dans le
    traçage et la personnalisation des graphiques.
"""
...
def format_spines(ax, right_border=True):
    """
    Cette fonction permet de mettre en place des bordures à partir d'un axe
    et de personnaliser les couleurs.
...
# Classe pour le tracé des étiquettes de données sur les diagrammes à barres.
# Référence: https://towardsdatascience.com/annotating-bar-charts-and-other-matplolib-techniques-cecb54315015
# Alias pour réduire la longueur des lignes.
...
# Définition de fonctions utiles pour tracer des étiquettes sur le graphique
def make_autopct(values):
    """
    Des pas :
        1. définition de la fonction de formatage des étiquettes

    Arguments :
        values -- valeurs extraites de la fonction value_counts() de la
        colonne d'analyse [list].

    Retourne :
        my_autopct -- chaîne formatée pour le tracé des étiquettes
    """
...
# Fonction permettant de tracer le graphique du fil en fonction d'une
# variable spécifique de l'ensemble de données.
def donut_plot(df, col, ax, label_names=None, text='', colors=['crimson', 'navy'], circle_radius=0.8,
            title=f'Graphique du fil', flag_ruido=0):
    """
    Étapes :
        1. définition des fonctions utiles pour afficher les étiquettes en
           valeur absolue et en pourcentage
        2. création d'une figure et d'un cercle central avec un rayon
           prédéfini
        3. Tracez le diagramme circulaire et ajoutez le cercle central.
        4. configuration finale de la parcelle

    Arguments :
        df  -- DataFrame cible de l'analyse [pandas.DataFrame].
        col -- colonne du DataFrame à analyser [string].
        label_names -- noms personnalisés à tracer comme étiquettes [list]
        text -- texte central à positionner [string / default : '']
        colors -- couleurs des entrées [list / default : ['cramoisi', 'marine']]]]
        figsize -- dimensions de la parcelle [tuple / défaut : (8, 8)]
        circle_radius -- rayon du cercle central [float / default : 0.8]

    Retourne :
        Aucune variable, juste une visualisation.
    """

    # Retour les valeurs et définition de la figure
...
# Fonction chargée de tracer la volumétrie d'une seule variable catégorielle
# dans un format actualisé.
def single_countplot(df, ax, x=None, y=None, top=None, order=True, hue=False, color='blue',
    ...
    if hue != False:
        if order:
            sns.countplot(x=x, y=y, data=df, color=color, ax=ax, order=df[col].value_counts().index, hue=hue)
        else:
            sns.countplot(x=x, y=y, data=df, color=color, ax=ax, hue=hue)
    else:
        if order:
            sns.countplot(x=x, y=y, data=df, color=color, ax=ax, order=df[col].value_counts().index)
        else:
            sns.countplot(x=x, y=y, data=df, color=color, ax=ax)
